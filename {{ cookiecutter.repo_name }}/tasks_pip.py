...
def lint(ctx: Context, *, fix: bool = True) -> None:
    """Run Ruff linter."""
    if fix:
        ctx.run("ruff check --fix src/ tests/", echo=True, pty=not WINDOWS)
    else:
        ctx.run("ruff check src/ tests/", echo=True, pty=not WINDOWS)

@task
def format(ctx: Context, *, fix: bool = True) -> None:  # noqa: A001
    """Run Ruff formatter."""
    if fix:
        ctx.run("ruff format src/ tests/", echo=True, pty=not WINDOWS)
    else:
        ctx.run("ruff format --check src/ tests/", echo=True, pty=not WINDOWS)

@task
def typecheck(ctx: Context) -> None:
    """Run MyPy type checker."""
    ctx.run("mypy src/ tests/", echo=True, pty=not WINDOWS)
